...
- 受管 route shells 和宿主 app root 下的 `/(auth-kit)/auth-config.ts` 会跟随 `app` / `src/app` 自动切换；`lib/billing/*` 和 `prisma/auth-kit.prisma` 会在 `src/app` 项目下自动生成到 `src/` 目录下，在普通 `app` 项目下保留根目录；`.env*` 始终固定在宿主项目根目录
- 套件默认生成 `lib/billing/order-redirect.ts`、`lib/billing/payment-success.ts` 和 `lib/billing/payment-finished.ts`；如果宿主使用的是 `src/app`，这些文件会分别生成到 `src/lib/billing/*`
