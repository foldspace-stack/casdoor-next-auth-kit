...
npx @foldspace-fe/casdoor-next-auth-kit@latest update   # 更新：重新生成路由壳、同步 skill 文件，并自动识别 app / src/app
npx @foldspace-fe/casdoor-next-auth-kit@latest check    # 检查：验证宿主项目配置与生成物是否一致，也会按自动识别的 app root 校验
```

`init` 会将整个 skill 目录复制到宿主项目的 `.agents/skills/` 目录，并按宿主当前使用的 app root（`app` 或 `src/app`）生成路由壳文件。`update` 用于套件升级后同步变更，并继续沿用同一个 app root。`check` 用于 CI 或手动验证，确保宿主项目未偏离套件的预期状态。

注意区分生成位置：

- 受管 route shells、`auth-config.ts`、callback 错误页等会跟随宿主 app root，写到 `app/(auth-kit)` 或 `src/app/(auth-kit)`
- `lib/billing/*`、`prisma/auth-kit.prisma`、`.env*` 仍然固定生成在宿主项目根目录，不会因为选择了 `app` 还是 `src/app` 而移动
...
- `init` 适合第一次接入宿主项目，缺少受管路由壳、`.env*`、`prisma/auth-kit.prisma`、`lib/billing/*` 或 skill 副本时使用
...
- 默认生成的宿主 app root 下 `/(auth-kit)/auth-config.ts` 是自包含的，能够在没有宿主数据库、`@/lib/db` 或额外权限模块的情况下直接编译；billing 处理器由套件默认生成到 `lib/billing/payment-success.ts` 和 `lib/billing/payment-finished.ts`，`auth-config.ts` 会直接导入这两个默认文件。若宿主需要落库、角色同步或自定义管理员策略，可以再在对应 custom block 里接入自己的实现
...
- 生成的宿主 app root 下 `/(auth-kit)/auth-config.ts` 默认是可编译的自包含实现；它只会依赖套件默认生成的 `lib/billing/payment-success.ts` 和 `lib/billing/payment-finished.ts`。如需接入宿主自己的数据库、角色判断或用户同步，可以在文件内的 custom block 里替换对应实现，而不是改动受管路由壳
