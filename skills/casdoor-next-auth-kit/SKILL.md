...
- 默认生成的 `app/(auth-kit)/auth-config.ts` 是自包含的，能够在没有宿主数据库、`@/lib/db` 或额外权限模块的情况下直接编译；如果宿主需要落库、角色同步或自定义管理员策略，可以再在该文件的 custom block 里接入自己的实现
...
- 生成的 `app/(auth-kit)/auth-config.ts` 默认是可编译的自包含实现，不会强制依赖宿主的 `@/lib/*`；如需接入宿主自己的数据库、角色判断或用户同步，可以在文件内的 custom block 里替换对应实现，而不是改动受管路由壳
