...
如果宿主项目使用的是本地 `file:` 依赖或工作区链接，先在本仓库执行 `pnpm build`，再回到宿主项目执行 `pnpm install` 或 `npx @foldspace/casdoor-next-auth-kit update`，否则可能看到旧产物。
...
当你执行 `npx @foldspace/casdoor-next-auth-kit update` 后，可以按下面顺序确认是否真的生效：
...
- Billing 相关接入优先导入 `@foldspace/casdoor-next-auth-kit/react` 和 `@foldspace/casdoor-next-auth-kit/billing`
