...
- `packages/auth-kit/src/core/index-html.ts` 还要在 `document.readyState` 可用时监听并替换 `#footer` 的 innerHTML；当 `window.DEFAULT_CASDOOR_POWERED_BY_HTML` 存在时，要把 `<footer id="footer">...</footer>` 内部替换成该 HTML 片段。
