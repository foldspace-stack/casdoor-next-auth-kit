...
- `packages/auth-kit/src/billing/casdoor-payment-session.ts`、`packages/auth-kit/src/billing/casdoor-plan-product.ts` 和 `packages/auth-kit/src/billing/casdoor-helpers.ts` 是可复用的 Casdoor 协议 helper，分别负责商品 checkout / QR payment / get-payment / notify-payment、pricing / plan / plan.product 解析、以及商品 ID / provider 归一化；它们只处理协议，不处理宿主订单、积分或会员落库。
...
- Casdoor 同源授权壳里如果出现 `/result/*` 路径，要隐式重写回 `/auth/login?redirect=%2F`，避免注册完成后把用户停在 Casdoor 的结果页。
...
- `packages/auth-kit/src/core/index-html.ts` 还要持续监听 `#footer` 的变化；当 `window.DEFAULT_CASDOOR_POWERED_BY_HTML` 存在时，只要 footer 被改动就重新把 `<footer id="footer">...</footer>` 的内部恢复成该 HTML 片段，并且要把监听收窄到 footer 专属 observer，写入时临时断开再恢复，避免全局监听造成 authorize 页初始化卡死或自触发死循环。
