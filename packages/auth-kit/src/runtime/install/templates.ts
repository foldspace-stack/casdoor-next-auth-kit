...
export function GET(request: Request) {
  ...
  // 不要删除这个路由，否则旧浏览器状态或 Casdoor SPA 回归会直接落到 404，授权 bootstrap 也没有机会把用户带回宿主个人中心。
...
import { paymentSuccessHandler as billingPaymentSuccessHandler } from './billing/payment-success';
import { paymentFinishedHandler as billingPaymentFinishedHandler } from './billing/payment-finished';
import { syncUserRecord } from './user-record';
...
export function userRecordTemplate() {
  return `import type { AuthUser } from '@foldspace-fe/casdoor-next-auth-kit';

export async function syncUserRecord(user: AuthUser): Promise<void> {
  console.info('[casdoor-next-auth-kit] syncUserRecord called', {
    userId: user.id,
    email: user.email,
    role: user.role,
